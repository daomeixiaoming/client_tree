...
export default class GridListView extends cc.Component {
    ...
    public delegate = new cc.Component.EventHandler();
    ...
    protected initParam() {
        ...
        let ui = this.content;
        let contentSize = ui.getContentSize();
        let tempSize = temp.getContentSize();
        ...
        if (this.ScrollView.vertical) { //垂直
            ...
            ui.setContentSize(new cc.Size(viewSize.width, contentSize.height));
            ui.setAnchorPoint(cc.v2(0.5, 1));
            this.content.setPosition(cc.v2(0, viewSize.height / 2));
            ...
            ui.setContentSize(new cc.Size(contentSize.width, viewSize.height));
            ui.setAnchorPoint(cc.v2(0, 0.5));
            this.content.setPosition(cc.v2(viewSize.width / 2, 0));
        ...
        if (this.ScrollView.vertical) {//垂直
            ui.setContentSize(new cc.Size(contentSize.width, this.contentSize));
        } else { //水平
            ui.setContentSize(new cc.Size(this.contentSize, contentSize.height));
        }
    ...
    protected addItems() {
        ...
        // cc.log('=============addItems2=================', this.items.length)
        if (this.items.length > 0) {
            // cc.log('=============addItems3============', this.items[0].position)
        }
    }
    ...
    protected getPositionInView(item) { // get item position in cc.ScrollView's node space
    ...
    protected updateItem() {
        ...
        for (let i = 0; i < this.items.length; i += this.cellNum) {
            ...
            if (items[i]) {
                // cc.log('=========updateItem==========',items[i].position)
                let viewPos = this.getPositionInView(items[i]);
                // cc.log('=========updateItem1==========',viewPos)
                // cc.log(' i =====  ', i)
                if (this.ScrollView.vertical) {
                    if (isDown) {
                        ...
                        if (viewPos.y > this.bufferZone && items[i].position.y - this.offset > -this.contentSize) {
                            this.position_front++;
                            this.position_back++;
                            // cc.log('======向下滑动=========',viewPos.y,this.bufferZone,items[i].position.y,this.offset,this.contentSize)
                            for (let j = 0; j < this.cellNum; j++) {
...
function log(arg0: string, visibleWidth: number, arg2: string, itemSize: number, arg4: string, cellNum: number) {
